...
void* carFunc(void* messageQueueStruct)
{
    ...
    MsgQueue* exitMsgQueuePtr = static_cast<MsgQueue*>(queueStruct->entryQ);
    MsgQueue* entryMsgQueuePtr = static_cast<MsgQueue*>(queueStruct->exitQ);
    MsgQueue carQ(10);    
    MsgQueue* carMsgQueuePtr = &carQ;
    
    //Start the car here, is done instead of manually from main. 
    carMsgQueuePtr->send(ID_CAR_SEND_OPEN);
    while (1) 
    {
        unsigned long id;
        //Receive message
        ...
        //Handle message
        carhandler(msg,id, carMsgQueuePtr, exitMsgQueuePtr, entryMsgQueuePtr, carId_);
